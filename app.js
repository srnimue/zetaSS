...
async function run() {
    ...
    try {
        ...
        // OCRは複数の前処理で行い、「読めた文章の中から対象文字を探す」。
        // ここでは黒塗り位置のロジックを変えず、認識・検索部分だけを強化する。
        const ocrModes = [
            {
                name: "通常",
                make: src => src
            ...
            {
                name: "二値化180",
                make: src => makeThresholdCanvas(src, 180)
            },
            {
                name: "二値化220",
                make: src => makeThresholdCanvas(src, 220)
            },
            {
                name: "反転",
                make: src => makeInvertCanvas(src)
            }
        ];

        function makeThresholdCanvas(src, threshold) {
            const out = document.createElement("canvas");
            out.width = src.width;
            out.height = src.height;
            const c = out.getContext("2d");
        ...
        function symbolChars(symbol) {
            const text = normalize(symbol?.text);
            if (!text || !symbol?.bbox) return [];
            // 通常は1symbol=1文字。複数文字を返した場合は同じbboxを共有する。
            return [...text].map(ch => ({ ch, bbox: symbol.bbox }));
        }

        function buildLineCharStream(line) {
            const stream = [];
            const words = (line?.words || []).filter(w => w?.bbox && normalize(w.text));
            for (const word of words) {
                const wordText = normalize(word.text);
                const symbols = (word.symbols || []).flatMap(symbolChars);
                if (symbols.length === wordText.length) {
                    stream.push(...symbols);
                    continue;
                }

                // symbol情報が欠けている場合はword bboxから文字位置を概算。
                const b = word.bbox;
                [...wordText].forEach((ch, i) => {
                    const x0 = b.x0 + (b.x1 - b.x0) * i / Math.max(1, wordText.length);
                    const x1 = b.x0 + (b.x1 - b.x0) * (i + 1) / Math.max(1, wordText.length);
                    stream.push({
                        ch,
                        bbox: { x0, y0: b.y0, x1, y1: b.y1 }
                    });
                });
            }
            return stream;
        ...
        function findBoxesInCharStream(stream, targetValue) {
        ...
        function lineTargetBoxes(line) {
            ...
            const stream = buildLineCharStream(line);
            const fromSymbols = findBoxesInCharStream(stream, target);
            if (fromSymbols.length) return fromSymbols;

            // 最後の保険。line bbox内の文字位置から対象部分を概算する。
            if (line?.bbox) {
                const boxes = [];
                const b = line.bbox;
                const len = Math.max(1, lineText.length);
                let from = 0;
                while (from <= lineText.length - target.length) {
                    const idx = lineText.indexOf(target, from);
                    if (idx < 0) break;
                    boxes.push({
                        x0: b.x0 + (b.x1 - b.x0) * idx / len,
                        y0: b.y0,
                        x1: b.x0 + (b.x1 - b.x0) * (idx + target.length) / len,
                        y1: b.y1
                    });
        ...
        function wordTargetBoxes(word) {
            const wordText = normalize(word?.text);
            if (!word?.bbox || !wordText.includes(target)) return [];
            const stream = (word.symbols || []).flatMap(symbolChars);
            const fromSymbols = findBoxesInCharStream(stream, target);
            if (fromSymbols.length) return fromSymbols;
            const boxes = [];
            const b = word.bbox;
            const len = Math.max(1, wordText.length);
            let from = 0;
            while (from <= wordText.length - target.length) {
                const idx = wordText.indexOf(target, from);
                if (idx < 0) break;
                boxes.push({
                    x0: b.x0 + (b.x1 - b.x0) * idx / len,
                    y0: b.y0,
        ...
        function dedupeMatches(matches) {
            ...
            for (const bbox of matches) {
                ...
                if (w < 2 || h < 2 || w > canvas.width * .8) continue;
        ...
        async function recognizeTarget(inputCanvas, modeName) {
            // PSM 11を基本にする。V19の実績を維持しつつ、認識結果はline単位でも検索する。
            ...
            for (const word of words) {
                matches.push(...wordTargetBoxes(word));
            }

            // 「ゆ」「ー」「ざ」「ー」のようにwordが分割されても、line全体を連結して検索する。
            for (const line of lines) {
                matches.push(...lineTargetBoxes(line));
            }
        ...
        for (const mode of ocrModes) {
            pass++;
            status(`OCR中… ${pass}/${ocrModes.length}\n${mode.name}で検索しています。`);
            const processed = mode.make(ocrCanvas);
            const result = await recognizeTarget(processed, mode.name);
            allResults.push(result);
        }

        const matches = allResults.flatMap(result => result.matches);
        status(`OCR候補を統合中…\n${allResults.filter(r => r.matches.length).length}/${ocrModes.length}種類の前処理で対象文字を発見しました。`);
        ...
        status(`黒塗り完了：${finalMatches.length}箇所\n${allResults.filter(r => r.matches.length).length}/${ocrModes.length}種類のOCRで候補を発見。\n取りこぼしがあれば「手動黒塗り」で追加できます。`);
